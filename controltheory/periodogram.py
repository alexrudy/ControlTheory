...
def periodogram(data, length=None, window=None, 
    ...
    :param ndarray window: The windowing function for the periodogram. If it
        is `None`, a standard windowing function will be used.
    :param bool half_overlap: Whether to half-overlap the segments of the
        periodogram.
    :param bool suppress_static: Whether to remove the static term from each 
        individual segment. This won't remove static bleed.
    :param bool mean_remove: Whether to remove the overal static term, aligning
        the magnitude of all PSDs.
    :param int skip_length: A length over which to skip data at the end of each
        periodogram. Useful for clipping poorly behaved sections.
    :param int clip_length: A length over which to skip data at the end of the
        timeseries. Useful for clipping poorly behaved sections.
    :param int start_length: A length over which to skip data at the start of
    ...
    if (not isinstance(skip_length, int)) or skip_length < 0:
        raise ValueError("Parameter skip_length must be a non-negative "
            "integer. Got {!r}".format(skip_length))
    if skip_length > 0 and length is None:
        raise ValueError("Cannot use skip_length and not specify a full "
            "periodogram length.")
    ...
    if length > total_length:
        raise ValueError("Periodogram cannot be longer than data: data={},"
            " periodogram={}".format(total_length, length))
    ...
    log.debug("Building a ({}) -> ({}) periodogram".format(
        "x".join(map("{:d}".format,data.shape)),
        "x".join(map("{:d}".format,psd_shape))))
...
def periodogram_slices(length, total_length, ndim, half_overlap=True, 
    ...
    if skip_length != 0 and half_overlap:
        log.warning("Ignoring half_overlap=True when "
...
def cosine_window(periodogram_length):
    ...
    where :math:`l` is the length of the periodogram, and :math:`x` is the 
    position along that periodogram.
